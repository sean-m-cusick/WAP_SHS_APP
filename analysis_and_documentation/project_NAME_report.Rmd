...
* **Kang Hin Lee** -  
  *"It is a very excited opportunity to practise everything I have learned from CodeClan so far using the live data from PHS. The aim for myself is to hone my data cleaning and wrangling skills and improve my analysis skills through working with my peers."*  
...
## Objective
The objective for our Minimum Viable Product (MVP) was to demonstrate the following topic: What, if any, __effect does the  Winter Season__ have on the acute Health Sector in Scotland, specifically __within Accident and Emergency (A&E)__; additionally, how does the __COVID-19 pandemic__ influence this further. 
...
Using these and our knowledge of RShiny, we sketched the areas in jam board to allow us to start building the wire frame.
...
### Data Cleaning
The data cleaning process is built based on computational & time efficiency, aimed to perform a generic clean to all dataset with a standard consistency across most of the datasets.

The datasets are first extracted from the PHS website using API keys then converted and stored at local repository as `.csv` files. 
The cleaning script is then loaded the datasets and performed generic cleaning process such as remove redundant columns and created a 'quarter' & 'year' columns throughout most datasets.
The cleaned datasets are then output into 'clean_script' folder for data analysis.

Additional cleaning may require on individual dataset depends on each analysis' requirement.
...
In order to plot the maps, spatial data which was gleaned from `.csv` files were stored as the following file types:
