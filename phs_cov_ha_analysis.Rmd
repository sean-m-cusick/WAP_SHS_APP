...
```{r}
hypo_season_all <- hypo_all_trend_hb_ans %>% 
  select(-pre_cov_adm_rate, - cov_adm_rate)
...
```{r}
hypo_season_spring <- hypo_season %>% 
  filter(quarter == c("1","2"))
```
```{r}
...
# Ha: mu_winter - mu_spring > 0
```{r}
hypo_season_summer <- hypo_season %>% 
  filter(quarter == c("1","3"))
```
```{r}
...
# Ha: mu_winter - mu_spring > 0
```{r}
hypo_season_autumn <- hypo_season %>% 
