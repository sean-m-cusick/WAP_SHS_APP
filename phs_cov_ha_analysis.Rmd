...
all_trend_hb_ans %>% 
  ...
  geom_rect(aes(xmin = week_ending, xmax = dplyr::lead(week_ending), ymin = -Inf, ymax = Inf, fill = factor(quarter)), alpha = .3, show.legend = TRUE) +
    scale_fill_manual(labels = c("Winter", "Spring", "Other"), values = alpha(c("steelblue", "green" ,"transparent"), .1))+
...
age_group_trend_hb_ans %>% 
  ggplot()+
    ...
    scale_fill_manual(values = alpha(c("steelblue", "transparent" ,"transparent" ,"transparent"), .4))+
