...
yrng <- range(all_trend_hb_ans$moving_avg_2021)
...
all_trend_hb_ans %>% 
  ...
  geom_rect(aes(xmin = week_ending, xmax = dplyr::lead(week_ending), ymin = -Inf, ymax = Inf, fill = factor(quarter), show.legend = FALSE), alpha = .4) +
    scale_fill_manual(values = alpha(c("steelblue", "white" ,"white" ,"white"), .3))+
  xlab("Time (date)")+
