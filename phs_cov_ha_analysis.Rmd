...
ca_name_list
    ...
    df_general_stat %>% 
      ...
      arrange(date) %>% 
      group_by(ca) %>% 
      filter(ca_name == "Clackmannanshire") %>% 
      slice_tail(n  = 1) %>% 
      ungroup() %>% 
      group_by(date) %>% 
      summarise(daily_positive = sum(daily_positive),
...
# write.csv(ca_name_list,"E:\\CodeClan\\phs_group_project\\phs_rshiny_dashboard_grp1\\clean_data\\df_ca_name_list.csv", row.names = FALSE)
```
# hypothesis testing
```{r}
